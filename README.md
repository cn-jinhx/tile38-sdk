...
## 目标要求
1. 能够具备通过Web/API进行POI数据导出，并能够根据POI进行电子围栏自动设定；
2. 能够实现多规则的电子围栏，如圆形、矩形与多边形；
3. 能够实现进出电子围栏的主动事件推送，可为WebHook、Kafka等形式；
4. 能够支持多租户控制，保障不同设备上报的定位信息归纳不同的地理围栏规则组中；
5. 能够支持私有化部署；
6. 基于以上电子围栏基础上额外支持其他自定义参数值的告警，如进入电子围栏后设定要求车速超过30KM/h推送自定义告警事件；
